...
class Moneyinfo:
    ...
    def __init__(self,filename,search_mode="1",category="1"):
        self.filename = filename             # 가져올 파일 이름 (use,plus,save,)
        self.search_mode = search_mode       # 1일(06.23), 1달(06) input 구별용
        self.category = category             # 저축 금액은 카테고리를 입력하지 않기 때문에 카테고리별 비중 출력 없이 금액만 출력.

    def cal_present(self,useAddr):           # 총 수입 - 총 지출:
        total1 = 0
        total2 = 0
